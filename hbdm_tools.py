...
@time: 07/03/2020 16:44
'''

from hbdm_requests import http_get_request, api_key_post
...
class HuobiDM:
    ...
    # 合约计划委托下单
    def send_plan_order(self, symbol, contract_type, contract_code, trigger_type,
                        trigger_price, order_price, order_price_type, volume,
        ...
        :contract_type: "this_week", "next_week", "quarter"
        :contract_code: "BTC181228"
        :trigger_type: 必填   String 触发类型： ge大于等于(触发价比最新价大)；le小于(触发价比最新价小)
        :trigger_price 必填   Number 触发价，精度超过最小变动单位会报错
        :order_price          Number 委托价，精度超过最小变动单位会报错
        :order_price_type     委托类型： 不填默认为limit; 限价：limit ，最优5档：optimal_5，最优10档：optimal_10，最优20档：optimal_20
        :volume         必填  委托数量(张)
        :direction         必填  "buy" "sell"
        :offset            必填   "open", "close"
        :lever_rate        必填  开仓必须填写，平仓单在有持仓的情况下可以不填。杠杆倍数[开仓若有10倍多单，就不能再下20倍多单]
        备注：如果contract_code填了值，那就按照contract_code去下单，如果contract_code没有填值，则按照symbol+contract_type去下单。
        :
        """
    ...
    # 获取合约历史成交
    def get_contract_history_trades(self, symbol, trade_type, create_date, contract_code=None,
                                    page_index=None, page_size=None):
        """
        参数名称     是否必须  类型     描述	    取值范围
        symbol      true	    string  品种代码  "BTC","ETH"...
        trade_type  true	    int     交易类型  0:全部,1:买入开多,2: 卖出开空,3: 买入平空,4: 卖出平多,5: 卖出强平,6: 买入强平,7:交割平多,8: 交割平空
        create_data true	    int     日期     可随意输入正整数，如果参数超过90则默认查询90天的数据
        page_index  false   int     页码，不填默认第1页
        page_size   false   int     不填默认20，不得多于50
        """
